...
# This allows dbcollect to run as non-privileged user (i.e., 'nobody'), see "using a logons file" below
dbcollect --logons /tmp/logons

# Specify the ORACLE_HOME
# Use if the OS user has no access to oratab/oracle inventory
dbcollect --logons /tmp/logons--orahome /u01/app/oracle/product/21.0.0/dbhome_1
```
...
```
echo '/usr/local/bin/dbcollect -o --logons /tmp/logons' | runuser nobody -s /bin/bash
# or (cleaner)
