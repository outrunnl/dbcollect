...
-- License     : GPLv3+
-- -----------------------------------------------------------------------------
-- This version modified for Oracle 10g and removed some sections
-- -----------------------------------------------------------------------------
-- Requires: Oracle database == 10g
--
...
FROM (
  ...
  UNION ALL SELECT 'REDOLOG',       bytes, NULL,  member fROM v$log a JOIN v$logfile b ON a.Group#=b.Group# -- No blocksize in 10g
)
ORDER BY 1, 2
/
...
-- , disk.os_mb                    ALLOCATED -- Does not exist in 10g
, disk.total_mb                    ALLOCATED
...
, NULL                     SNAPFLAG  -- Does not exist in 10g
, COUNT(*)                 SNAPSHOTS
