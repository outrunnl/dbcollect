...
select
  ...
  max(cts.encrypted) "ENC",
  max(cts.compress_for) "CMP",
  --
  substr(
...
from cdb_tablespaces cts, 
  ...
  (
    ...
    union all
    select
      nvl(cts.con_id,0) con_id,
      cts.tablespace_name, 0
    from cdb_tablespaces cts
    where not exists ( 
      select 1 
      from cdb_free_space dfs 
      where 1=1 
      and nvl(dfs.con_id,0) = nvl(cts.con_id,0) 
      and dfs.tablespace_name = cts.tablespace_name 
    )
  ) my_free_space
where 1=1
--and cts.contents not in ('TEMPORARY')
and cts.con_id = my_dba_data_files.con_id(+)
and cts.tablespace_name = my_dba_data_files.tablespace_name(+)
and cts.tablespace_name = my_free_space.tablespace_name(+)
group by 
  cts.contents, 
  cts.block_size,
  cts.encrypted, 
